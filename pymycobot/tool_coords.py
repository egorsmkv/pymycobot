...
def CvtRotationMatrixToEulerAngle(pdtRotationMatrix):
    ...
    pdtEulerAngle[2] = np.arctan2(
        pdtRotationMatrix[1, 0], pdtRotationMatrix[0, 0]
    )
    ...
    pdtEulerAngle[1] = np.arctan2(
        ...
        (fCosRoll * pdtRotationMatrix[0, 0])
        + (fSinRoll * pdtRotationMatrix[1, 0]),
    )
    pdtEulerAngle[0] = np.arctan2(
        (fSinRoll * pdtRotationMatrix[0, 2])
        - (fCosRoll * pdtRotationMatrix[1, 2]),
        (-fSinRoll * pdtRotationMatrix[0, 1])
        + (fCosRoll * pdtRotationMatrix[1, 1]),
    )
...
def flangeToTool(current_coords, tool_matrix):
    ...
    flange_pose[:3, :3] = CvtEulerAngleToRotationMatrix(
        current_coords[3:6] * np.pi / 180.0
    )
    ...
    tool_coords = np.concatenate(
        (
            tool_pose[:3, 3].T,
            CvtRotationMatrixToEulerAngle(tool_pose[:3, :3]) * 180 / np.pi,
        )
    )
...
def toolToflange(tool_coords, tool_matrix):
    ...
    tool_pose = np.eye(4)
    tool_pose[:3, :3] = CvtEulerAngleToRotationMatrix(
        tool_coords[3:6] * np.pi / 180.0
    )
    ...
    flange_coords = np.concatenate(
        (
            flange_pose[:3, 3].T,
            CvtRotationMatrixToEulerAngle(flange_pose[:3, :3]) * 180 / np.pi,
        )
    )
