...
class MyWindow(QWidget, Ui_Form):
    def __init__(self):
        ...
        self.icon = QIcon()
        self.icon.addPixmap(
            QPixmap(os.getcwd() + "/resources/mystudio.ico"),
            QIcon.Normal,
            QIcon.Off,
        )
        ...
        self.switch_button.status_button.connect(
            partial(self.control_serial, 1)
        )
        self.horizontalLayout.addWidget(self.switch_button)

        self.switch_button_1 = SwitchButton()
        self.switch_button_1.status_button.connect(
            partial(self.control_serial, 2)
        )
        self.horizontalLayout_2.addWidget(self.switch_button_1)

        self.switch_button_2 = SwitchButton()
        self.switch_button_2.status_button.connect(
            partial(self.control_serial, 3)
        )
        self.horizontalLayout_3.addWidget(self.switch_button_2)

        self.switch_button_3 = SwitchButton()
        self.switch_button_3.status_button.connect(
            partial(self.control_serial, 4)
        )
            ...
            if self.comboBox.currentText() != "":
                if status:
                    if self.serial_obj[index] is not None:
                        ...
                        self.serial_obj[index] = CreateSerial(
            ...
            if self.comboBox_3.currentText() != "":
                if status:
                    if self.serial_obj[index] is not None:
                        ...
                        self.serial_obj[index] = CreateSerial(
                            self.comboBox_3.currentText(), index
                        )
                        self.serial_obj[index].progress.connect(self.add_row)
                        self.serial_obj[index].start()
                else:
            ...
            if self.comboBox_4.currentText() != "":
                if status:
                    ...
                    if self.serial_obj[index] is not None:
                        ...
                        # print("create4")
                        self.serial_obj[index] = CreateSerial(
                            self.comboBox_4.currentText(), index
                        )
                        self.serial_obj[index].progress.connect(self.add_row)
                        self.serial_obj[index].start()
                else:
