...
from core.controller import (
    UndefinedController,
    MyCobot630Controller,
    MyCobot320Controller,
    MyArmMControl,
)
...
# ##############################################################################################
COMPOSITE_KIT_TYPE = (
    "Undefined"  # MyCobotPro630, MyCobot320, MyArmM750, Undefined
)
DEBUG = False
...
def main():
    ...
    if COMPOSITE_KIT_TYPE == "MyCobotPro630":
        print(
            f" # Connecting to MyCobot Pro630 at {COMPOSITE_KIT_HOST}:{COMPOSITE_KIT_PORT}..."
        )
        arm_controller = MyCobot630Controller(
            host=COMPOSITE_KIT_HOST, port=COMPOSITE_KIT_PORT, debug=DEBUG
        )

    elif COMPOSITE_KIT_TYPE == "MyCobot320":
        print(f" # Connecting to MyCobot 320 at {COMPOSITE_KIT_COMPORT}...")
        arm_controller = MyCobot320Controller(
            port=COMPOSITE_KIT_COMPORT, debug=DEBUG
        )
    ...
    composite_kit = AGVProCompositeKit(
        arm_controller=arm_controller, agv_pro_driver=agvpro_driver
    )
